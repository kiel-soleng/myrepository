...
3. **Parent the KPIs + charts to the LINKED INPUT TABLE** (`source.elementId` = the
   ...
   > **Correction (2026-08-24, sigma-office-of-finance consuming this skill):**
   > sourcing a `kpi-chart`/chart directly from the input table produced a masked
   > `Invalid kind: "input-table"` on PUT, pointing at the input-table element
   > itself. The exemplar this file points to (`build_cava.py`'s page 2 / the
   > simpler `build_demand_planning_lite.py`) does NOT actually do this —
   > `build_demand_planning_lite.py` inserts one plain `table` ("Book" — *"the
   > single downstream read surface every chart/KPI uses"*) sourced from the
   > input table, redeclares every needed column, and points every KPI/chart at
   > that instead. Follow the exemplar, not this bullet, until this is
   > re-verified: parent to a derived Book table, not the input table directly.
